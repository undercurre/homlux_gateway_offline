...
export const useRouteStore = defineStore('route-store', {
  state: (): RouteState => ({
    ...
    searchMenus: [],
    cacheRoutes: [],
    isProjectDetailOpened: false
  }),
  actions: {
    /** 项目详情打开情况 */
    initIsProjectDetailOpened() {
      const data = localStg.get('isProjectDetailOpened');
      this.isProjectDetailOpened = Boolean(data);
      this.handleOpenProjectDetail(this.isProjectDetailOpened);
    },
    /** 项目详情打开情况 */
    cacheIsProjectDetailOpened() {
      localStg.set('isProjectDetailOpened', this.isProjectDetailOpened);
    },
    /** 项目详情打开处理 */
    handleOpenProjectDetail(isOpen: boolean) {
      const projectStore = useProjectStore();
      this.isProjectDetailOpened = isOpen;
      this.cacheIsProjectDetailOpened();
      const auth = useAuthStore();
      let routes = filterAuthRoutesByUserPermission(staticRoutes, auth.roleLevel);
      routes = routes.map(routeItem => {
        if (/project-detail-/.test(routeItem.name)) {
          if (isOpen && projectStore.isLockProject && routeItem.meta.type === 'lock') {
            routeItem.meta.hide = false;
          } else if (isOpen && !projectStore.isLockProject && routeItem.meta.type === 'light') {
            routeItem.meta.hide = false;
          } else {
            routeItem.meta.hide = true;
          }
        } else if (!/setting/.test(routeItem.name)) routeItem.meta.hide = isOpen;
        return routeItem;
      });
    ...
    async initDynamicRoute() {
      ...
      const { mobilePhone } = localStg.get('userInfo') || {};

      if (!mobilePhone) {
        throw new Error('mobilePhone 不能为空!');
      }

      const { error, data } = await fetchUserRoutes(mobilePhone);
    ...
    async initStaticRoute() {
      ...
      const routes = filterAuthRoutesByUserPermission(staticRoutes, auth.roleLevel);
